...
public class Modern 
{
	private String modernSets[]=new String[100];
	private String modernNames[]=new String[300];
	int pointer=0;
	int year=0;
	public void start()
	{
	...
	public void tcg()
	{
		try
		{
			...
			HSSFWorkbook modern = new HSSFWorkbook();
			HSSFSheet setname;
			Row newRow;
			Cell info;
			...
			for(int limit=0; limit<this.pointer; limit++)
			{
				...
				setname=modern.createSheet(modernNames[limit]);
				...
				System.out.println("\nSet name: "+modernNames[limit]+"\n");
				//Each modified string value is then put in the following url to then parse
				//the information from it.
				page = Jsoup.connect("http://magic.tcgplayer.com/db/price_guide.asp?setname="+modernSets[limit]).get();
			...
			File modernFile = new File("modern.xls");
			FileOutputStream modernOutput = new FileOutputStream(modernFile);
			modern.write(modernOutput);
			modernOutput.close();
	...
	public boolean modern()
	{
		...
		try
		{
			...
			Document page = Jsoup.connect("https://www.wizards.com/Magic/TCG/Resources.aspx?x=judge/resources/sfrmodern").get();
			...
			{
				modernNames[pointer]=item.text();
				System.out.println(modernNames[pointer]);
				...
				{
					modernSets[pointer]=coreSet(clean);
					
				}
				else
				{
					modernSets[pointer]=clean;
				}
